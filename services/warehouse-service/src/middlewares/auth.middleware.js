...
export async function authenticate(req, res, next) {
  ...
  try {
    ...
    console.error('Error connecting to auth-service in Warehouse Service:', err.message);
...
export function requireWarehouseRole(req, res, next) {
  ...
  if (roles.includes('KHO') || roles.includes('QUAN_LY')) {
    return next();
  }

  return res.status(403).json({ message: 'Không có quyền truy cập. Yêu cầu quyền KHO hoặc QUAN_LY.' });
}
