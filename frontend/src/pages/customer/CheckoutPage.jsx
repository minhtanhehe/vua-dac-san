...
import { MapPin, Plus, Loader2, CreditCard, DollarSign, Sparkles, X } from 'lucide-react';
...
export default function CheckoutPage() {
  ...
  // Read cart from the central store — no local state or localStorage needed
  ...
  const checkoutMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      clearCart(); // Use store action instead of manual localStorage removal
      navigate('/');
    },
  ...
  const handleAddAddressSubmit = (e) => {
    ...
    addAddressMutation.mutate({
      ...
      laMacDinh: customer?.diaChi?.length === 0 // Make default if it's the first address
    });
  };
  ...
  const handlePlaceOrder = () => {
    ...
    const payload = {
      ...
      maKhuyenMai: couponCode.trim() || null,
  ...
  // BUG FIX 2.2: Ngăn chặn Đặt hàng nếu tài khoản bị khóa
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-brand-light p-6 shadow-sm space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <label
                  className={`flex items-center gap-3 p-4 rounded-xl border cursor-pointer transition ${paymentMethod === 'COD'
                    ? 'border-brand-primary bg-brand-primary/5'
                    : 'border-brand-light hover:bg-gray-50'
                    }`}
                >
                ...
                <label
                  className={`flex items-center gap-3 p-4 rounded-xl border cursor-pointer transition ${paymentMethod === 'Chuyển khoản'
                    ? 'border-brand-primary bg-brand-primary/5'
                    : 'border-brand-light hover:bg-gray-50'
                    }`}
                >
              ...
              {/* Show banking info if bank transfer */}
              {paymentMethod === 'Chuyển khoản' && (
                <div className="bg-blue-50 border border-blue-200 rounded-xl p-5 text-sm text-blue-800 space-y-3">
                  ...
                  <p className="text-[10px] text-gray-400 italic">
                    * Đơn hàng sẽ được bộ phận Bán hàng duyệt ngay sau khi nhận được tiền chuyển khoản thành công.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="bg-white rounded-2xl border border-brand-light p-6 shadow-sm space-y-6">
                    ...
                    <div key={productId} className="py-3 flex justify-between gap-4 text-xs">
                      <div className="flex-1 space-y-0.5">
                        ...
                        <div className="text-gray-400">
                          {item.quantity} {productUnit} x {formatVND(productPrice)}
                        </div>
                      </div>
                      <span className="font-semibold text-brand-dark text-right">
                        {formatVND(item.quantity * productPrice)}
                      </span>
                    </div>
                  );
                })}
              ...
              <div className="pt-4 border-t border-brand-light space-y-2">
                <label className="text-xs font-bold text-brand-dark flex items-center gap-1.5">
                  <Sparkles size={14} className="text-brand-primary" /> Mã giảm giá (Mã KM)
                </label>
                <div className="flex gap-2">
                  <input
                    value={couponCode}
                    onChange={e => setCouponCode(e.target.value.toUpperCase())}
                    placeholder="Mã giảm giá (ví dụ: FREESHIP)"
                    className="flex-1 px-3 py-2 bg-brand-bg/50 border border-brand-light rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-brand-primary focus:bg-white uppercase"
                  />
                </div>
              </div>

              {/* Pricing details */}
              <div className="space-y-2.5 pt-4 border-t border-brand-light text-xs text-gray-500">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-brand-dark">{formatVND(shippingFee)}</span>
                </div>
              </div>
              ...
              <div className="border-t border-brand-light pt-4 flex justify-between items-baseline">
                ...
                <span className="text-xl font-black text-brand-accent font-heading">
                  {formatVND(total)}
                </span>
              </div>
